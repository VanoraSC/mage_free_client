...
| 0071 | Offer Pass when priority is held but no fresh prompt exists | 0052, 0057, 0070 | **Defect found live (Pete, 2026-08-16).** `controlsFor` returns no controls at all (`state.prompt ?: return null`) whenever there is no live prompt — but a 0070-restored rejoin snapshot never carries `prompt` (only a live push does, by `GameViewMapper.apply`'s own explicit design), so a player correctly shown as holding priority after rejoining has no way to Pass. `sendPlayerAction`/`PASS_PRIORITY_*` is confirmed out-of-band (not tied to answering a specific prompt), so the fix is a client-side fallback control, not a protocol change. |
