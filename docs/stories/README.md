...
| 0068 | Bridge transport security and access control | 0001, 0005 | **Parked — a future increment, not blocking current work.** Traced from the actual code: the bridge speaks plain `ws://` only (no TLS connector exists at all), binds to a hardcoded `0.0.0.0`, and has no bridge-level auth — anyone who can reach the port can attempt a session. Correctly non-blocking while the bridge only runs on a developer's own trusted LAN; formalizes a review-follow-up note that previously had no story number. |
