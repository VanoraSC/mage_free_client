...
| 0068 | Bridge TLS via nginx reverse proxy | 0001, 0005 | **Scope resolved (Pete): nginx reverse proxy for TLS termination; bridge-level access control explicitly out of scope (accepted risk).** The bridge speaks plain `ws://` only today (no TLS connector exists). Adds an `nginx` service to `docker/docker-compose.yml` terminating `wss://` and proxying the WebSocket upgrade to `bridge:8080`, stops publishing the bridge's port directly to the host, and picks a cert-trust approach for a real device. Formalizes a review-follow-up note that previously had no story number. |
