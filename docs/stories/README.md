...
| 0073 | Stack fan layout, tap-to-inspect, and target visualization | 0051, 0057 | **Board design specified directly by Pete (2026-08-16).** Fans the stack ~60% overlapping instead of side-by-side, and adds tap-to-inspect: dim what's not involved, draw an arrow to the tapped ability's target(s), show a card view (real art, ability text substituted for rules text when it's an ability). Covers targets on the stack/battlefield only — anything else needs a separately designed visualization later. Grounded in upstream's own `CardView.getTargets()` (never mapped by this bridge) and the desktop client's own target-arrow tooltip, confirmed by reading the pinned source directly — not new design, data upstream already sends. |
