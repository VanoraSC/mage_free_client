...
| 0085 | Robolectric out of the logic modules | 0082, 0083 | Seven of the ten Robolectric tests in the `:core:*` logic modules become plain JVM tests on the `jvm()` target, which `./gradlew check` runs — that is what makes the portability claim check itself rather than resting on a grep. The other three test the Android edge itself and stay, as do the 5 Compose tests — they are the hermetic gate. |
