...
- Story 0003 makes `:bridge` depend on `org.mage:mage-common:1.4.60` (the XMage client session
  ...
  reactor via Maven (`mvn install`), which also builds the large card database. See
  [`../architecture.md`](../architecture.md) and [`../build-environment.md`](../build-environment.md).
- The upstream targets an older Java; whether it builds cleanly on **JDK 17** in-container is an
  open question this story resolves empirically (it may need build flags or a specific JDK).
- **Pin the upstream version.** The bridge is version-locked to the server (architecture Decision #6),
  so the mage build must be pinned to a specific commit/tag (baseline: `1.4.60`).

## 3. Scope

**In scope**
- A Dockerfile layer/stage (on the 0020 base) that: obtains `magefree/mage` at a **pinned ref**
  (clone a tag/commit — reproducible, CI-friendly), runs `mvn -DskipTests install`, and leaves
  `mage-common` (+ transitive `org.mage:*`) in the image's local Maven repo (`~/.m2`).
- A pin file / build arg for the upstream ref (e.g. `MAGE_REF=1.4.60` as a `--build-arg`).
...
- Requires 0020. Uses Maven (from 0020) + JDK 17. Pin the upstream `mage` ref to `1.4.60` (matching
  the bridge's target). Expect a long first build (card DB) — it is cached in the image layer.
- If JDK 17 can't build upstream cleanly, document the exact fix (flags, or a second pinned JDK in
  the layer) rather than working around it silently.

## 5. Design & approach

- **Layer:** `ARG MAGE_REF=1.4.60`; `git clone --depth 1 --branch ${MAGE_REF} https://github.com/magefree/mage`;
  `mvn -q -DskipTests install` from the reactor root; then the clone can be removed while the
  installed artifacts remain in `~/.m2`. Keep this as its own layer so it caches independently of
  day-to-day changes.
- **Resolution:** downstream Gradle/Maven builds in the container use `mavenLocal()` to find
  `org.mage:mage-common:1.4.60`.
