 ...
 *
 * **Seats do not come from here.** XMage emits no per-seat callback before match-start, so
...
internal object TableEventFold {
     ...
     *   advance to [TablePhase.Starting] (the game boundary).
     */
    fun fold(
        state: TableState,
     ...
     * server state may have changed**, so the observer should re-read it. These are the
     ...
     * Appending a brand-new seat instead would mean a repeated null-id push grew
