plugins {
    ...
    id("magefree.koin")
    // KSP used to arrive with the Hilt convention plugin. Hilt is gone, but Room's
    ...
    // kotlinx.serialization parses the bundled formats.json legality asset.
...
// KSP writes Room's generated `DeckDatabase_Impl`/`DeckDao_Impl` into the `androidDebug`
    ...
    sourceSets {
        commonMain {
            dependencies {
                ...
                // never touches the catalog data. There is a second reason: `BundledFiles`,
                ...
                // room-runtime is the multiplatform artifact from 2.7. room-ktx is gone
                ...
                // FormatBundleLoader reads okio Sources off the BundledFiles boundary.
        ...
        /**
         * the deck-persistence suite runs here, on the `jvm()` target, over Room's
        ...
        // `AndroidSQLiteDriver`, APK assets — so they stay on Android by design.
        androidUnitTest {
            dependencies {
                implementation(libs.junit4)
                implementation(libs.kotlinx.coroutines.test)
...
dependencies {
    // Room's processor runs per Kotlin target, not once for the module — each target
