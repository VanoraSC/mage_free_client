...
dependencyResolutionManagement {
    ...
    repositories {
        ...
        // /root/.m2 and consumed only by :bridge. Declared here because the repo runs
        // FAIL_ON_PROJECT_REPOS, which forbids a project-level repositories {} block; the
...
if (System.getenv("MAGE_JVM_ONLY") != "1") {
    ...
    // bundled XMage card catalog + offline local search (Android lib; needs the SDK).
    include(":core:cards")
    // offline deck model, local deck library (Room) + bundled format-legality (Android lib).
    include(":core:decks")
    include(":feature:connect")
    include(":feature:lobby")
    // card search/browse + full-bleed inspection UI over :core:cards (Android lib).
