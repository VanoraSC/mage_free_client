 ...
 * `SessionStateCode` path through the real WebSocket + [SessionCoordinator] plumbing.
 *
 * The scripted statuses are the *upstream* view; the coordinator stamps the login's `requestId` onto
 * the first one, so scripts should carry a null `requestId`.
 */
public class FakeUpstreamSession(
    private val script: List<SessionStatus>,
) : UpstreamSession {
